...
<table>
  ...
  <tbody>
    <tr>
      <td>Area of Interest</td>
      <td>The area where the algorithm requests the Sentinel-2 data and detects trucks. Note that the algorithm requests **ALL** Sentienl-2 data available inside of area of interest no matter there are roads or not.</td>
    </tr>
    <tr>
      <td>Time range</td>
      <td>The time range that the algorithm requests the Sentinel-2 data and detects trucks. Note that the algorithm requests **ALL** available data/acquisitions within the time range.</td>
    </tr>
    <tr>
      <td>OSM Values</td>
      <td>The road types that the algorithm detects trucks. Currently motorway, trunk, and primary are supported. The OSM Values determine the valid area and mask data outside of selected roads.</td>
    </tr>
    <tr>
      <td>Max. cloud coverage</td>
      <td>The algorithm computes the cloud coverage of the pixels located over the network of roads (pixel intersecting roads) and computes the % of cloudiness by dividing the number of cloudy pixels by the total number of pixels intersecting roads. There is a trade-off between a high value that can generate a lot of false positives because of the same signature of cloud edges as trucks, and a low value that can loose a lot of traffic detected resulted from filtering too much.</td>
    </tr>
    <tr>
      <td>Week days</td>
      <td>The algorithm allows users to select specific week days for filtering purposes. Note that the revisit time of Sentinel-2 is 5 days, meaning that there may be only ONE acquisition for each week day in a month. To avoid empty result, the less week days are selected, the longer time range is required.</td>
    </tr>
   </tbody>
</table>  
